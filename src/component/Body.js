...
const kanbanData = require("../kanban");
let data = kanbanData;
export default class Body extends Component {
  ...
  renderList() {
    ...
    for (let key in this.state.kanban) {
      activityList.push(
        <Activityboard
          backlogData={this.state.kanban[key]}
          listName={key}
          key={key}
          onDropEvent={this.onDropEvent}
          createCardBody={this.createNewCard}
          updateCard={this.updateCard}
          deleteCardData={this.deleteCard}
          deleteList={this.deleteList}
        />
      );
    }
    return <div className="body-scroller">{activityList}</div>;
  }
  toggleModal() {
    this.setState({
      openModal: true,
